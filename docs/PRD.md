...
`prompt-memory`는 Claude Code, Codex 같은 AI 코딩 도구에 사용자가 입력한 프롬프트를 로컬에 안전하게 기록하고, 다시 찾고, 분석하고, 다음 요청을 더 잘 쓰도록 돕는 developer tool이다.

제품 포지셔닝은 "AI coding prompt memory and improvement workspace, local-first"다. 한국어로는 "AI 코딩 프롬프트를 로컬에서 기억하고, 다시 찾고, 더 나은 요청으로 고쳐 쓰게 돕는 도구"로 정의한다.

구현 관점에서 `prompt-memory`는 지원하는 AI 코딩 도구의 사용자 프롬프트를 자동으로 수집하고, Markdown 파일로 저장하며, 로컬 웹 UI에서 검색, 조회, 분석, 삭제, 개선안 복사를 할 수 있게 하는 오픈소스 도구다.
...
- 개선 중심: 단순 아카이브가 아니라 프롬프트 코치처럼 다음 입력을 더 좋게 만드는 피드백 루프를 제공한다.
- 패턴 회고: 사용자가 반복해서 넣는 좋지 않은 프롬프트 패턴을 드러내고, 더 명확한 목표, 맥락, 제약, 검증 요청으로 바꾸도록 돕는다.
