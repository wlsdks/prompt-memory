...
Phase 2의 목적은 `prompt-memory`를 "안전하게 저장하고 찾는 도구"에서 "프로젝트별로 운영하고, 과거 기록을 가져오고, 사용자의 좋지 않은 프롬프트 패턴을 분석하며, 더 나은 prompt를 개선해 다시 쓰는 도구"로 확장하는 것이다.

이 단계의 제품 정체성은 "AI coding prompt memory and improvement workspace, local-first"다. 즉 Claude Code, Codex 같은 AI 코딩 도구에 입력한 프롬프트를 로컬에 안전하게 기록하고, 다시 찾고, 분석하고, 다음 요청을 더 잘 쓰도록 돕는 developer tool로 발전시킨다.
