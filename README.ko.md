...
2. MCP를 등록해 agent 세션 안에서 coach/rewrite/judge tool을 호출할 수 있게 합니다.
3. 실제 코딩 요청을 하나 보냅니다.
4. `prompt-memory doctor <tool>`와 `prompt-memory coach`로 첫 점수와 개선 제안을 확인합니다.
5. dashboard/search/history가 필요할 때만 로컬 archive를 엽니다.
...
# MCP 등록이 실패하거나 --register-mcp를 생략한 경우:
# claude mcp add --transport stdio prompt-memory -- prompt-memory mcp
# 또는: codex mcp add prompt-memory -- prompt-memory mcp
# 실제 Claude Code 또는 Codex prompt를 하나 보냅니다
