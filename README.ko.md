...
설치 후 `prompt-memory setup --profile coach`를 실행하면 핵심 흐름은 다음과 같습니다.

1. Claude Code 또는 Codex prompt를 로컬에 캡처합니다.
2. `prompt-memory doctor <tool>`로 캡처 상태를 확인합니다.
3. agent 안에서 최신 요청을 점수화하거나 개선안을 받습니다.
4. dashboard/search/history가 필요할 때만 로컬 archive를 엽니다.

대부분의 사용자는 아래 흐름으로 시작하면 됩니다.
...
- 외부 LLM 분석: 숨은 provider 호출 없음. 명시적으로 요청한 경우에만 현재
  사용자가 제어하는 Claude Code/Codex 세션이 평가하도록 MCP agent-judge
  packet을 제공
- 기본 데이터 처리: 로컬 전용
...
- 외부 LLM 분석은 숨은 background 호출로 실행되지 않습니다. 선택형 MCP
  agent-judge workflow는 사용자가 요청한 경우에만 redacted prompt packet을
  현재 user-controlled agent 세션에 제공합니다.
