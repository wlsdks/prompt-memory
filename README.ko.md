...
프롬프트 상세 화면은 로컬 rule-based analysis preview를 제공합니다. 목표, 배경 맥락, 범위 제한, 출력 형식, 검증 기준이 포함되어 있는지 요약합니다.

이 preview는 저장된 redacted prompt body를 로컬에서만 분석합니다. 외부 LLM provider를 호출하지 않습니다.

## Benchmark

Benchmark v1은 privacy, retrieval, rule-based prompt improvement, analytics, latency에 대한 로컬 회귀 신호를 측정합니다.
