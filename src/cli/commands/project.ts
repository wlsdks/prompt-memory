...
export function showProjectForCli(options: ProjectShowCliOptions): string {
  return withProjectStorage(options.dataDir, ({ storage, hmacSecret }) => {
    ...
    if (!summary) {
      throw new UserError(
        `No captured project matches cwd "${options.cwd}". Capture at least one prompt from this directory first, or run prompt-memory project list to see known projects.`,
      );
    }
