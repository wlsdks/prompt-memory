...
export function diagnoseIngestFailure({
  ...
  if (status !== 401) {
    return {
      cause: "unknown",
      hint: "Run prompt-memory buddy --once to inspect the most recent failed hook ingest.",
    };
  }
  ...
  if (
    ...
    return {
      ...
      hint: `A different server is bound to port ${port} using data dir ${ownerDataDir} instead of ${configuredDataDir}. Stop it and start the configured service: prompt-memory service stop && prompt-memory service install.`,
    };
  }
  ...
  if (
    ...
    return {
      ...
      hint: "The local server keeps exiting with a Node ABI mismatch (better-sqlite3 NODE_MODULE_VERSION error). Reinstall the LaunchAgent with the current Node binary: prompt-memory service install.",
    };
  }

  return {
    cause: "token_stale",
    hint: `Reinstall the hook to refresh the local ingest token: prompt-memory install-hook ${tool}.`,
  };
}
