...
- Agent judge 기능은 현재 사용자가 제어하는 Claude Code/Codex/Gemini CLI 세션이 MCP로 redacted packet을 받아 평가하는 opt-in 흐름만 허용한다. `prompt-coach`는 provider 인증정보를 추출, 저장, proxy하거나 숨은 외부 LLM 호출을 하지 않는다.
